...
const WORK_ORDER_TO_APPOINTMENT: Record<string, string> = {
  waiting: 'confirmed',
...
for (const apt of appointments) {
  ...
  if (!wo) {
    console.log(`⚠️  预约#${apt.id} 没有对应工单，正在创建...`);
  ...
  const expectedAptStatus = WORK_ORDER_TO_APPOINTMENT[wo.status];

  if (!expectedAptStatus) {
    console.log(`⚠️  工单#${wo.id} 状态「${wo.status}」无映射规则，跳过`);
    skippedCount++;
    continue;
  }

  if (apt.status !== expectedAptStatus) {
    console.log(
    ...
    console.log(`✅ 预约#${apt.id} 状态一致 (${apt.status})`);
  }
}
...
console.log(`  状态一致: ${appointments.length - fixedCount - createdWOCount - skippedCount - errorCount}`);
